...
### Fixed
- **Web UI now shows the real running version.** The header badge and `/api/profile` / status endpoints were hardcoded (`v2.13.1` / `2.12.0` / `2.9.0`) and never wired to `package.json`; all three now use `PACKAGE_VERSION`. Also removed a stale `better-sqlite3` step from `make update-internal` (migrations run automatically on startup via `node:sqlite`).
